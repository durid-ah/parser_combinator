...
/// the mapped result
pub fn map_result<'a, R1, R2, T, E1, E2, S, F, I>(mut parser: I, map_fn: F) -> Box<dyn Parse< R1, S, T, E1, E2> + 'a>
   where F: Fn(ParserResult<R2, E2>) -> ParserResult<S,E2> + 'a,
...
pub fn chain_parser<'a,I,F,R1,R2,T,E1,E2,S>(mut parser: I, chain_fn: F) -> Box<dyn Parse<R1, S, T, E1, E2> + 'a>
   where F: Fn(& ParserResult<R2, E2>) -> Box<dyn Parse<R2, S, T, E2, E2> + 'a> + 'a,
         I: Parse<R1,R2,T,E1,E2> + 'a,
         R1: 'a, R2: 'a, T: 'a, E1: 'a, E2: 'a, S: 'a {
